...
- `WithTrustedProxies` - `ClientIP` 解析使用的可信代理 CIDR 列表（默认：`["0.0.0.0/0","::/0"]`，与 Gin 保持一致）。
- `WithRemoteIPHeaders` - 真实客户端 IP 解析头优先级（默认：`["X-Forwarded-For","X-Real-IP"]`）。
- `WithCORSPolicy` - 可配置 CORS 策略（默认：启用且偏保守，不默认全放开）。
...
### 2.1 客户端 IP 转发与可信代理策略

`EnableForwardedByClientIp` 用于控制是否启用基于转发头的真实客户端 IP 解析。

当启用 `EnableForwardedByClientIp` 时：

1. 默认与 Gin 保持一致，信任所有代理（`0.0.0.0/0`、`::/0`）。
2. 生产环境建议显式通过 `WithTrustedProxies(...)` 收紧可信边界。
3. 如果你显式设置了 `WithTrustedProxies(...)` 且值非法（IP/CIDR 格式错误），`orbit` 会在启动阶段 fail-fast 并中止启动。
4. `WithRemoteIPHeaders(...)` 用于控制 `ClientIP()` 的头部解析优先级。
...
func main() {
	// 生产环境示例：启用 forwarded 头解析，并且只信任内网代理。
	config := orbit.NewConfig().
		WithTrustedProxies([]string{"10.0.0.0/8"}).
		WithRemoteIPHeaders([]string{"X-Forwarded-For", "X-Real-IP"}) // 可选，默认即为该顺序

	// 启用基于 forwarded 头的 ClientIP 解析。
...
> [!NOTE]
>
> - 如果你设置 `WithTrustedProxies([]string{})`，即使启用了 `EnableForwardedByClientIp`，也会忽略 forwarded 头。
> - 若在启用 `EnableForwardedByClientIp` 的情况下设置了非法 CIDR（例如 `invalid-cidr`），引擎初始化会失败，`Run()` 将中止启动。

### 2.2 CORS 策略

`orbit` 现已使用显式 CORS 策略配置。

1. 默认策略更保守，不会自动放开所有浏览器来源。
2. 可通过 `WithCORSPolicy(...)` 配置来源白名单、方法、头部与凭证策略。
3. 服务间调用（非浏览器场景）可显式关闭 CORS。
...
> [!TIP]
>
> 若面向浏览器且需要携带凭证（cookies/Authorization），更推荐使用来源白名单（`AllowedOrigins`）并返回具体 Origin（即保持 `AllowAllOrigins=false`）。
...
`orbit` 基于 Gin，可按普通 HTTP 路由方式实现 WebSocket 升级处理。

### 13.1 最小 WebSocket 示例
...
### 13.2 反向代理/LB 注意事项（云无关）

1. 确保代理链路保留 `Upgrade` 与 `Connection: upgrade` 头。
2. 代理与服务端超时要大于心跳周期，避免空闲断连。
3. 会话保持策略要与 WebSocket 状态模型一致（优先无状态 Hub 设计）。
4. 若启用 `EnableForwardedByClientIp`，生产环境建议显式配置 `WithTrustedProxies(...)`。
