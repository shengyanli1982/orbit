...
import "github.com/shengyanli1982/orbit/utils/log"

// 包含了一个用于管理日志事件对象池的 sync.Pool
type LogEventPool struct {
	eventPool *ObjectPool[*log.LogEvent] // 日志事件对象池
}

// 返回一个新的 LogEventPool 实例
func NewLogEventPool() *LogEventPool {
	return &LogEventPool{
		// 创建一个新的对象池，用于复用 LogEvent 对象
		eventPool: NewObjectPool(func() *log.LogEvent {
			return &log.LogEvent{}
		}),
	}
}
